...
const getSchemaError = (schema = joi.object(), objectToValidate = {}) => {
  const { error } = schema.validate(objectToValidate, { allowUnknown: true });
...
const getValidator = (schemaName, schema) => async (req, res, next) => {
  ...
  SCHEMA.REQUEST_TYPES.forEach(type => {
    if (schema[type]) {
      ...
      const error = getSchemaError(schema[type], req[type]);
      error && schemaError.push(error);
    }
  });
