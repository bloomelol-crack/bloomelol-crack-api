 ...
 */
export const Find = (key: string, where = {}): Promise<{ string: string[] | null; json: object[] | null }> =>
  new Promise(resolve => {
    redis.smembers(key, (error, docs) => {
      ...
      const results: { string: string[]; json: object[] } = { string: [], json: [] };
 ...
 * @param {object[]} registers Registers to add
 */
export const Add = (key: string, registers: object[]): Promise<number | null> =>
  new Promise(resolve => {
    const regs = Array.isArray(registers) ? registers : [registers];
    const strRegs = regs.map(reg => JSON.stringify(reg));
    redis.sadd(key, ...strRegs, (error, added) => {
      if (error) {
...
export const Set = (
  ...
  value: any,
  expire: number = 0
): Promise<{ added: string; expired: number } | null> =>
  new Promise(resolve => {
 ...
 */
export const Delete = (key: string, where: any = {}): Promise<number | null> =>
...
export const Update = (
  ...
  strRegisters?: string[] | null
): Promise<number | null> =>
  new Promise(async resolve => {
    let regs: { [key: string]: any }[] | null;
    if (strRegisters) {
    ...
    redis.srem(key, ...strRegisters, async (error, deleted) => {
      ...
      for (let k = 0; regs && k < regs.length; k += 1)
